...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Benchmark for LLM models in Exo adapted for Jetson boards')
    ...
    # New board argument: 'nano' or 'orin-agx'
    parser.add_argument('--board', choices=['nano', 'orin-agx'], default='orin-agx', help='Jetson board type')
...
def main():
    ...
    # Adjust parameters based on the board type
    if args.board == 'nano':
        # Jetson Nano is slower so increase timeout and reduce number of runs
        args.timeout = max(args.timeout, 600)
        args.runs = 1 if args.runs > 1 else args.runs
        print("Board detected: Jetson Nano (using increased timeout and reduced runs)")
    else:
        print("Board detected: Jetson Orin AGX (using default parameters)")
